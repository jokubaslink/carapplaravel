<div class="z-10 bg-gray-500 bg-opacity-40 overflow-hidden absolute top-0 left-0 bottom-0 right-0 ">
    <form
        class="flex flex-col items-end justify-end gap-8 bg-gray-100 p-8 max-w-[450px] sm:max-w-[500px] md:max-w-[600px] lg:max-w-[800px] w-full max-h-[800px] h-full absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
        action="{{route('rented.car', $car)}}"
        method="get">
        <a href="{{route('route.car', $car['id'])}}" class="absolute top-4 text-red-500 font-bold text-2xl">X</a>
        <figure class="m-auto">
            <img src="{{$car['img']}}" alt="">
        </figure>
